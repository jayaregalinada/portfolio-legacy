...
{
    return gulp.src( elixir.config.assetsDir + 'images/**/*.+(jpg|png|bmp|gif|jpeg)' )
        .pipe( imagemin({
            progressive: true,
            optimizationLevel: 5,
            interlaced: true
        }).on('end', function(){
            $.util.log( 'Images compressed!' );
        }))
        .pipe( gulp.dest( paths.dist.images ) )
        .on('end', function(){
            $.util.log( 'Images are in dist! Congrats!' )
        });
});
...
elixir(function(mix) {
    ...
    bowerVendors.jsDashboard.forEach( function(val, key) {
        bowerVendors.jsDashboard[key] = paths.vendorDir + val;
    });
    bowerVendors.css.forEach(function(val, key) {
        bowerVendors.css[key] = paths.vendorDir + val;
    });
    bowerVendors.cssDashboard.forEach(function(val, key) {
        bowerVendors.cssDashboard[key] = paths.vendorDir + val;
    });
